...
while True:
    ...
    if rev["post_type"] == "message":
            ...
            if rev["message_type"] == "group" or rev["message_type"] == "private": #群聊和私聊
                ...
                if "[CQ:at,qq="+QQID+"]" in message:
                    ...
                    if people == '薄荷':
                        ...
                        send_msg({'msg_type':str(messagetype),'number':sendid,'msg':'骑骑!'})
                    elif people == "狰豹":
                        ...
                        send_msg({'msg_type':str(messagetype),'number':sendid,'msg':'[CQ:image,file=file:///'+os.getcwd()+r'/turka_qiqi.jpg]'})
                    elif people == "啾町":
                        send_msg({'msg_type':str(messagetype),'number':sendid,'msg':'哇伊阿啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊！'})
                    else:
                    ...
                    if  messagecommand[0:2] == "找找":
                        ...
                        findfriendjson = findluo2tuyafind(findluoV4tantan(FindLuoID,FindLuoKey,name))
                        if findfriendjson.get("code") == 0:
                            if 'state' in findfriendjson:
                                if findfriendjson['data']['state'] == 0:
                                    #state：0 = 正常，1 = 审核中（返回状态码 2101），-1 待回收（返回状态码 2002），-2 角色已被管理员锁定（返回状态码 2102）
                                    #-1 一般当 404 处理
                                    #不过state一般不用管，因为状态异常会返回报错，可以参考返回状态码处理
                                    findluo_state = "正常滴"
                                elif findfriendjson['data']['state'] == 1:
                                    findluo_state = "审核中"
                                elif findfriendjson['data']['state'] == -1:
                                    findluo_state = "待回收"
                                elif findfriendjson['data']['state'] == -2:
                                    findluo_state = "已锁定"
                                else:
                                    findluo_state = "未知呜"
                            if findfriendjson['data']["gender"] == 0:
                                #"gender" = 0 : 女孩几 uwu,"gender" = 1 : 男孩纸 OwO,"gender" = 0 : 是男孩纸哩还是女孩几里？OwU
                                #gender 性别：0 = 女，1 = 男，2 = 其他性别
                                #findluo_gender = "女孩几 uwu" if findfriendjson['data']["gender"] == 0 else "男孩纸 OwO"
                                findluo_gender = "女孩几 uwu"
                            elif findfriendjson['data']["gender"] == 1:
                                findluo_gender = "男孩纸 OwO"
                            elif findfriendjson['data']["gender"] == 2:
                                findluo_gender = "是男孩纸哩还是女孩几里？OwU"
                            else:
                                findluo_gender = "咦？系奇怪的性别 ?w?"
                            ...
                            tuyafindtext = "[CQ:face,id=147]"+str(findfriendjson.get('data').get('name'))+'（ID：'+str(findfriendjson.get('data').get('id'))+'）'+str('\n')+"[CQ:face,id=144]破壳："+str(findfriendjson.get('data').get('birthday'))+str('\n')+"[CQ:face,id=75]家乡："+str(findfriendjson.get('data').get("hometown"))+str('\n')+"[CQ:face,id=185]物种："+str(findfriendjson.get('data').get("animal"))+str('\n')+"[CQ:face,id=12]性格："+str(findfriendjson.get('data').get("cha"))+str('\n')+"[CQ:face,id=66]喜欢："+str(findfriendjson.get('data').get("likes"))
                            findluotext = "------涂鸦研究所报告------\n[CQ:face,id=147]"+str(findfriendjson.get('data').get('name'))+'（ID：'+str(findfriendjson.get('data').get('id'))+"）"+str('\n')+"[CQ:face,id=144]破壳："+str(findfriendjson.get('data').get('birthday'))+str('\n')+"[CQ:face,id=75]家乡："+str(findfriendjson.get('data').get("hometown"))+str('\n')+"[CQ:face,id=185]物种："+str(findfriendjson.get('data').get("animal"))+str('\n')+"[CQ:face,id=66]性别："+str(findluo_gender)+str('\n')+"[CQ:face,id=142]关键词："+str(findfriendjson.get('data').get("keywords"))+"\n---------洛浔找找---------" 
                            time.sleep(0.1)
                            send_msg({'msg_type':str(messagetype),'number':sendid,'msg':findluotext})
                            time.sleep(0.1)
                            """
                            ...
                            try:
                                ...
                                #pass
                            """
                        elif findfriendjson.get("code") == 2001 or findfriendjson.get("code") == 1002:
                            send_msg({'msg_type':str(messagetype),'number':sendid,'msg':"[CQ:face,id=106]呜呜，我好像没有找到他呢，该不会是没来过？(或者叫找找找找?)QwQ"+str('\n')+"-------------------"+str('\n')+MintBotVersion})
                        else:
                            print("[CQ:face,id=9]呜呜,失败了qwq:("+str(findfriendjson.get("code"))+"):"+findfriendjson.get("msg")+str('\n')+"-------------------"+str('\n')+MintBotVersion)
                            send_msg({'msg_type':str(messagetype),'number':sendid,'msg':"[CQ:face,id=9]呜呜,失败了xwx"})
                        '''
                        findfriend = messagecommand[4:len(messagecommand)]
                                                                    ...
                                                                    if qq == searchweatherQQ:
                                                                        if message.isdigit() == True:
                                                                            if int(message) <= int(swlistlen)-1:
                                                                                ...
                                                                                messageRefresh = 0
                                                                                break    
                                                                        elif message[0:len(BotName)+5] == BotName+"，停止操作" or message[0:len(BotName)+5] == BotName+",停止操作" or message[0:len(BotName)+5] == BotName+"，取消操作" or message[0:len(BotName)+5] == BotName+",取消操作":
                                                                            swlistlen=swlistlen+1
                        ...
                        print(MoId)
                        MoApiQQimageUrl = WebMediaApi.QQHeaderImgBTSTU(MoId)
                        MoQQimagegetJson = demjson.decode(MoApiQQimageUrl)
                        print(MoQQimagegetJson)
                        Mofilejsonget = WebMediaApi.MoMoWerPlus(MoQQimagegetJson.get('imgurl'))
                    ...
                    elif messagecommand[0:2] == '踢我':
                        if str(rev["group_id"]) == "114514":
                            if str(qq) == '114514':
                                send_msg({'msg_type':str(messagetype),'number':sendid,'msg':f'[CQ:at,qq={str(qq)}]彳亍'})
                                if cqhttpaccesstoken == '':
                                    print(requests.get(f'http://{str(cqhttpserverip)}:{str(cqhttpserveriphost)}/set_group_kick?group_id=114514&user_id=114514').content)
                                else:
                                    print(requests.get(f'http://{str(cqhttpserverip)}:{str(cqhttpserveriphost)}/set_group_kick?group_id=114514&user_id=114514&access_token={str(cqhttpaccesstoken)}').content)
                            else:
                                send_msg({'msg_type':str(messagetype),'number':sendid,'msg':f'[CQ:at,qq={str(qq)}]？'})
                        else:
                            send_msg({'msg_type':str(messagetype),'number':sendid,'msg':f'[CQ:at,qq={str(qq)}]？'})
                        ...
                        if str(FCMcalendar_list[0][int(time.strftime(r"%d",time.localtime()))-1]) == "1":
                            ...
                            send_msg({'msg_type':str(messagetype),'number':sendid,'msg':f"[CQ:at,qq={qq}] [CQ:face,id=12]今天可以游玩一小时！！！不过还有{lastday}天就结束了xwx"})
                        else:
        ...
        if rev["request_type"] == 'friend':
            ...
            can_be_add = 0 #是否启用加群通过，具体实现请看残留代码和go-cqhttp文档
            #continue
            if cqhttpaccesstoken == '':
                if rev["sub_type"] == 'add' and can_be_add == 1:
                    if str(rev["group_id"]) == "114514":
                        requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=add&approve=true')
                    elif str(rev["group_id"]) == "114514":
                        print(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))])
                        if str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))].isdigit() == True:
                            print('获取到的会员证为纯数字')
                            if len(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))]) != 10:
                                print("会员证非10位数")
                                requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=add&approve=false&reason=无法查询到会员证ID，无法加群。')
                                continue
                            #BCXxjm = json.loads(BCMCreator.user_info_xjm_Plus_local(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))]), strict=False)
                            BCXxjm = json.loads(BCMCreator.user_info_xjm_Plus(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))]), strict=False)
                            print('已获取到数据:'+str(BCXxjm.get("code")))
                            if str(BCXxjm.get("code")) == "200":
                                print('返回数据正确')
                                if str(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))]) in bcxVIPCode:
                                    print('该会员证已被使用')
                                    print(requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=add&approve=false&reason=该会员证ID已被使用，无法再次加群，请联系管理申诉。'+'&access_token='+str(cqhttpaccesstoken)).content)
                                else:
                                    print('该会员证未被使用')
                                    requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=add&approve=true')
                                    time.sleep(1)
                                    bcxVIPCode.append(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))])
                                    save_save()
                                    send_msg({'msg_type':'group','number':str(rev["group_id"]),'msg':"欢迎新会员加入此群！\n会员昵称:"+str(BCXxjm.get("name"))+str("\n")+"[CQ:at,qq="+str(rev["user_id"])+"]，欢迎加入本协会，欢迎进入本群。\n如何增加经验值：投稿作品（Kitten/Nemo/Python/Coco/Box3）\n本群传统：\n1.AMC的深夜食堂，指晚上的时候会发一些美食照片\n2.吃冷鱼，冷鱼是鱼，特级保护动物，濒危只剩一个，大家都想方设法的想吃它，但未能如愿，但@薄荷本兽机器人在某人的操作下吃到了它，可喜可贺！但冷鱼却经常诈尸。。。\n本群机器人：薄荷本兽。\n可以通过群里发送 #菜单 获取工具。\n会员后台https://bcmcreator.cn/index.php?mod=main\n赛事平台https://competition.bcmcreator.cn/\n本群群规https://bcmcreator.cn/qg.php\n请不要乱刷此消息，如有看见，男盗女娼！\n文案by:@赖纸哦耶"})
                            else:
                                print('返回数据错误')
                                requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=add&approve=false&reason=无法查询到会员证ID，无法加群。')
                        else:
                        ...
                        continue
                elif rev["sub_type"] == 'invite':
                    requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=invite&approve=true')
                ...
                if rev["sub_type"] == 'add' and can_be_add == 1:
                    if str(rev["group_id"]) == "114514":
                        requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=add&approve=true'+'&access_token='+str(cqhttpaccesstoken))
                    elif str(rev["group_id"]) == "114514":
                        print(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))])
                        if str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))].isdigit() == True:
                            print('获取到的会员证为纯数字')
                            if len(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))]) != 10:
                                print("会员证非10位数")
                                requests.get('http://'+str(cqhttpserverip)+':'+str(cqhttpserveriphost)+'/set_group_add_request?flag='+rev["flag"]+'&sub_type=add&approve=false&reason=无法查询到会员证ID，无法加群。'+'&access_token='+str(cqhttpaccesstoken))
                                continue
                            BCXxjm = json.loads(BCMCreator.user_info_xjm_Plus(str(rev["comment"])[str(rev["comment"]).rfind('：')+1:len(str(rev["comment"]))]), strict=False)
                            print('已获取到数据:'+str(BCXxjm.get("code")))
                            if str(BCXxjm.get("code")) == "200":
